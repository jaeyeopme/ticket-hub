...
class JwtProvider(
    ...
    private val accessSecretKey = Keys.hmacShaKeyFor(jwtProperties.accessKey.toByteArray(Charsets.UTF_8))

    private val refreshSecretKey = Keys.hmacShaKeyFor(jwtProperties.refreshKey.toByteArray(Charsets.UTF_8))
    ...
    override fun generateAccessToken(tokenPayload: TokenPayload): String = generateToken(
        accessSecretKey,
        jwtProperties.accessExp,
        tokenPayload.claims(),
    )

    override fun generateRefreshToken(tokenPayload: TokenPayload): String = generateToken(
        refreshSecretKey,
        jwtProperties.refreshExp,
        tokenPayload.claims(),
    )

    override fun validateAccessToken(token: String?): TokenPayload = validateToken(token, accessSecretKey)

    override fun validateRefreshToken(token: String?): TokenPayload = validateToken(token, refreshSecretKey)
